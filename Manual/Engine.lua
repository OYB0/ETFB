    ...
    while task.wait(0.1) do
        if API.State.AutoFarmEnabled and not API.State.IsMoving then
            ...
            if target and target.Parent then
                -- [تحديد المسار الدقيق بناءً على طلبك]
                local rp = target:FindFirstChild("Root")
                local prompt = rp and rp:FindFirstChild("TakePrompt")
                
                -- نظام بديل احتياطي (Fallback) في حال كان الهدف Lucky Block ومساره مختلف
                if not rp or not prompt then
                    rp = target:FindFirstChild("Root") or target:FindFirstChild("RootPart") or target:FindFirstChildOfClass("Part") or target:FindFirstChildOfClass("MeshPart")
                    if not rp and target:IsA("BasePart") then rp = target end 
                    prompt = target:FindFirstChildOfClass("ProximityPrompt", true) or (rp and rp:FindFirstChild("TakePrompt"))
                end
