...
local function createSetting(text, default, key)
    ...
    b.FocusLost:Connect(function() 
        local val = tonumber(b.Text) or default
        API.Config[key] = val
        -- التحديث الفوري للحالة حتى ينطبق التعديل أثناء الفارم مباشرة
        if API.State[key] ~= nil then API.State[key] = val end 
...
createSetting("Farm Depth",   API.Config.FarmDepth or 1, "FarmDepth")
